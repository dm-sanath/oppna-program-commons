...
public class USDServiceImpl implements USDService {
	...
	private static final String[] attributeNamesContactCrList = new String[] {
			"description", "summary", "status", "ref_num", "web_url", "type" };
	private static final String[] attributeNamesContactChgList = new String[] {
			"description", "summary", "status", "chg_ref_num", "web_url" };
	...
	private void createAttachment(USD_WebServiceSoap service, int sid,
			String repHandle, String objectHandle, String description,
		...
		// Create attachment
		service.createAttachment(sid, repHandle, objectHandle, description,
				fileName);
	...
	private String getGroupHandle(String groupName) {
		...
		int sessionID = 0;
		String sql = "type = 2308 and delete_flag = 0 and last_name = '"
				+ groupName + "'";

		try {
			sessionID = getWebService().login(wsUser, wsPassword);
			String result = getWebService().doSelect(sessionID, "cnt", sql, -1,
			...
			throw new RuntimeException(
					"Error when parsing xml response when searching for a group",
					e);
		}

	}

	public String createRequest(Properties requestParameters, String userId,
			List<File> files, List<String> filenames) {
		...
		try {
			...
			for (Enumeration<Object> e = requestParameters.keys(); e
					.hasMoreElements();) {
			...
			result = getWebService().createRequest(sessionID, contactHandle,
					attrVals, propertyValues, template, attributes,
			...
			try {
				...
				throw new RuntimeException(
						"Error parsing handle to USD incident from xml response...\n"
								+ result, e);
			}
					...
					for (File f : files) {
						try {
							if (filenames != null && filenames.size() > 0) {
								File newFile = new File(f.getParentFile(),
										filenames.get(i));
								boolean renameSuccess = f.renameTo(newFile);
								if (renameSuccess) {
									f = new File(f.getParentFile(), filenames
											.get(i));
								}
							}
							// System.out.println("Attaching: " +
							// f.getAbsolutePath());
							this.createAttachment(getWebService(), sessionID,
									wsAttachmentRepHandle, handle,
									"Attachment " + i, f.getAbsolutePath());
						} catch (Exception e) {
							throw new RuntimeException(
									"Error creating attchment in USD", e);
						}
						i++;
					}
				}
	...
	protected USD_WebServiceSoapSoapBindingStub getWebService() {
		if (webService == null) {
			...
			try {
				webService = new USD_WebServiceSoapSoapBindingStub(endPoint,
						service1);
			} catch (Exception e) {
				throw new RuntimeException(e);
			}
	...
	public String getUSDGroupHandleForApplicationName(String appName) {
		...
		if (result == null || result.length() == 0) {
			...
			throw new RuntimeException(
					"No group handle found for application name=" + appName);
		}
		return result;
	}

	private List<Issue> getIssuesForContact(String userId, Integer maxRows) {
		...
		try {
			...
			try {
				...
				log
						.error("Could not get handle for user with userId "
								+ userId);
			...
			// Get list xml
			String listXml = getWebService().doSelect(sessionID, "iss",
					whereClause.toString(), endIndex,
					attributeNamesContactCrList);

			// Get input stream for xml data
			ByteArrayInputStream bais = new ByteArrayInputStream(listXml
					.getBytes());
			...
			log.error("Failed to get issue list from USD Service for user="
					+ userId, e);
			try {
				getWebService().logout(sessionID);
			} catch (RemoteException e1) {
	...
	private List<Issue> getRequestForContact(String userId, Integer maxRows) {
		List<Issue> taskList = new ArrayList<Issue>();
		...
		try {
			...
			// Get list xml
			String listXml = getWebService().doSelect(sessionID, "cr",
					whereClause.toString(), endIndex,
					attributeNamesContactCrList);
	...
	private List<Issue> getChangeOrdersForContact(String userId, Integer maxRows) {
		List<Issue> taskList = new ArrayList<Issue>();
		...
		try {
			...
			// Get list xml
			String listXml = getWebService().doSelect(sessionID, "chg",
					whereClause.toString(), endIndex,
	...
	protected List<Issue> getIssuesFromList(InputStream xml)
		...
		try {
			...
			XPathExpression udsObjectXPath = xpathprocessor
					.compile(xpathUDSObject);

			// Execute the XPath expressions
			NodeList udsObjects = (NodeList) udsObjectXPath.evaluate(doc,
			...
			for (int i = 1; i < udsObjects.getLength() + 1; i++) {
				...
				xPath.append(i);
				xPath
						.append("]/Attributes/Attribute[AttrName='summary']/AttrValue");

				XPathExpression attributeValueXPath = xpathprocessor
						.compile(xPath.toString());
				String value = (String) attributeValueXPath.evaluate(doc,
				...
				xPath.append(i);
				xPath
						.append("]/Attributes/Attribute[AttrName='description']/AttrValue");

				attributeValueXPath = xpathprocessor.compile(xPath.toString());
				value = (String) attributeValueXPath.evaluate(doc,
						XPathConstants.STRING);
				...
				xPath.append(i);
				xPath
						.append("]/Attributes/Attribute[AttrName='status']/AttrValue");
				...
				xPath.append(i);
				xPath
						.append("]/Attributes/Attribute[AttrName='ref_num']/AttrValue");
				...
				xPath.append(i);
				xPath
						.append("]/Attributes/Attribute[AttrName='web_url']/AttrValue");
			...
			throw new RuntimeException(
					"Error when trying to parse issue list from XML", e);
		}
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see se.vgr.usdservice.USDService#getRecordsForContact(java.lang.String,
	 * java.lang.Integer)
	 */
	public List<Issue> getRecordsForContact(String userId, Integer maxRows) {
		List<Issue> records = new ArrayList<Issue>();
		List<Issue> changeOrdersForContact = getChangeOrdersForContact(userId,
				maxRows);
		List<Issue> requestForContact = getRequestForContact(userId, maxRows);
		List<Issue> issuesForContact = getIssuesForContact(userId, maxRows);
	...
	private String getHandleForUserid(int sessionID, String userId)
			throws RemoteException {
		return new StringBuffer(getWebService().getHandleForUserid(sessionID,
				userId)).toString();
	}

	public String createRequest(Properties testParameters, String string,
			List<File> files) {
